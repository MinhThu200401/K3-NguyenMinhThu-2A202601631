    ...
    def check_metrics(self) -> list[Alert]:
        ...
        if snap["block_rate"] > self.block_rate_threshold:
            self.alerts.append(Alert(
                ...
                message=f"Block rate {snap['block_rate']:.2%} exceeds threshold {self.block_rate_threshold:.2%}",
            ))
        ...
        if snap["judge_fail_rate"] > self.judge_fail_rate_threshold:
            self.alerts.append(Alert(
                ...
                message=f"Judge fail rate {snap['judge_fail_rate']:.2%} exceeds threshold {self.judge_fail_rate_threshold:.2%}",
            ))
